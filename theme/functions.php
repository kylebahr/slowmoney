...
function display_author_name()
{
	...
	$author_name = get_field('author_name', $product->get_id());

	if (!empty($author_name)) {
		echo '<h2 class="mt-2 text-sm text-gray-500">' . esc_html($author_name) . '</h2>';
	}
}
add_action('woocommerce_single_product_summary', 'display_author_name', 6);

function display_author_bio()
{
	global $product;
	$author_bio = get_field('author_bio', $product->get_id());
	$author_name = get_field('author_name', $product->get_id());
	...
	if (!empty($author_bio)) {
		...
		echo wpautop($author_bio) . '</div>';
	}
}
add_action('woocommerce_single_product_summary', 'display_author_bio', 30);
