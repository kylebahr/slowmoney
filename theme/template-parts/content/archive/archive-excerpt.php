	...
	<div class="flex-grow">
		...
		<div class="relative flex items-center mt-8 gap-x-4">
			...
			if ('post' === get_post_type()) :
				// Retrieve the selected author object
				$guest_author = get_field('author');

				if ($guest_author) :
					$author_id = $guest_author->ID;
					$author_name = get_field('guest_author_name', $author_id);
					$author_description = get_field('guest_author_bio', $author_id);
					$author_image_arr = get_field('guest_author_headshot', $author_id);
					$author_image_url = $author_image_arr['url'];

					// If author name and description exists, display author details
					if ($author_name && $author_description) :
			?>
						<div class="flex-shrink-0 block not-prose group">
							<div class="flex items-center">
								<div class="bg-gray-100 rounded-full">
									<img src="<?php echo esc_url($author_image_url); ?>" alt="<?php echo esc_attr($author_name); ?>" class="inline-block w-10 h-10 rounded-full">
								</div>
								<div class="ml-3 text-sm leading-6">
									<p class="font-semibold text-gray-900"><?php echo esc_html($author_name); ?></p>
									<p class="text-gray-600"><?php echo wp_kses_post($author_description); ?></p>
								</div>
							</div>
						</div>
					<?php
